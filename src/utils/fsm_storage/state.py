...
# https://github.com/aiogram/aiogram/blob/dev-2.x/aiogram/dispatcher/filters/state.py
class State:
    """
    State object
    """
    ...
    async def set(self):
        state = User.get_current().current_state()
...
class StatesGroup(metaclass=StatesGroupMeta):
    ...
    async def next(cls) -> str:
        state = await User.get_current().current_state()
    ...
    async def previous(cls) -> str:
        state_name = await User.get_current().current_state
